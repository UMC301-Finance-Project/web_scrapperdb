...
def calculate_means_per_ticker(df: pd.DataFrame):
    ...
    ret_dict = dict()  # Dictionary to store the result
    ...
    # Step 1: Calculate mean sentiment scores for each aspect, excluding zero values
    ...
    # Step 2: Concatenate original DataFrame with the computed averages
    #df = pd.concat([df, aspect_averages], axis=1)

    # Step 3: Create a dictionary with average scores per ticker
    for aspect in aspects:
        for ticker in list(get_nifty_50_tickers()):
            # Calculate the mean sentiment score for the current ticker and aspect, excluding zero values
            aspect_score_avg = mean_ignore_zeros(df[df['stock_symbol'] == ticker][aspect])
            
            # Initialize the ticker's dictionary if it does not exist
            if ticker not in ret_dict:
                ret_dict[ticker] = dict()
            
            # Assign the computed average score to the dictionary
            ret_dict[ticker][aspect] = aspect_score_avg
...
def main():
    ...
    for col in cols:
            if col == 'ticker':
                new_dict[col] = list(get_nifty_50_tickers())
            else:
                new_dict[col] = list(parameter[ticker][col] for ticker in list(get_nifty_50_tickers()))
    
    df2 = pd.DataFrame(new_dict)
    save_to_sqlite_and_firestore(df2)
